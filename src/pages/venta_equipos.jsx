...
export default function venta_equipos() {
    ...
    const savedPage = localStorage.getItem('products_venta_current_page');
  ...
  async function enviar(){
    ...
    async function get_all_products() {
      try {
        const { data } = await axios.get('https://backrecordatoriorenta-production.up.railway.app/api/products/read_venta');
    ...
    async function get_products_paginates(page = current_page) {
      try {
        const { data } = await axios.get(`https://backrecordatoriorenta-production.up.railway.app/api/products/read_pag_venta?page=${page}`);
        
        // Validamos si la página solicitada excede el total de páginas
        if (page > data?.totalPages) {
          page = data.totalPages; // Ajustamos a la última página
          localStorage.setItem('products_venta_current_page', page); // Actualizamos localStorage
        }
    
        setPaginacion(data);
        setTotal_pages(data?.totalPages);
        setProductos_paginados(data.response); // Mostramos los datos de la página ajustada
        setLoading(false); // Datos cargados, actualizamos el estado de carga
      } catch (error) {
  ...
  //FUNCION CUANDO DAS ENTER EN EL BUSCADOR
    ...
    function handlePageChange(pageNumber) {
      ...
      localStorage.setItem('products_venta_current_page', pageNumber);
 ...
 <div className="fixed inset-0 bg-black bg-opacity-75 flex  justify-center items-center z-50">
   ...
   <path d="M7.978 4a2.553 2.553 0 0 0-1.926.877C4.233 6.7 3.699 8.751 4.153 10.814c.44 1.995 1.778 3.893 3.456 5.572 1.68 1.679 3.577 3.018 5.57 3.459 2.062.456 4.115-.073 5.94-1.885a2.556 2.556 0 0 0 .001-3.861l-1.21-1.21a2.689 2.689 0 0 0-3.802 0l-.617.618a.806.806 0 0 1-1.14 0l-1.854-1.855a.807.807 0 0 1 0-1.14l.618-.62a2.692 2.692 0 0 0 0-3.803l-1.21-1.211A2.555 2.555 0 0 0 7.978 4Z"/>
       ...
       <div className="hidden lg:flex justify-between items-center">
         {/* Logo */}
         <button onClick={() => { localStorage.setItem('products_venta_current_page', 1), window.location.reload(); }}>
   ...
   <path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M5 7h14M5 12h14M5 17h14"/>
       ...
       {isOpen && (
         <div className="lg:hidden flex flex-col items-center gap-2 py-2 bg-[#C70000] border-t-[1px] mt-[1rem] border-solid ">
           ...
           <a 
             onClick={openFormulario} 
             className="text-white py-2 border-2 border-white rounded-[5px] px-[1.8rem] flex gap-2"><svg className="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
             <path fill-rule="evenodd" d="M12 2a7 7 0 0 0-7 7 3 3 0 0 0-3 3v2a3 3 0 0 0 3 3h1a1 1 0 0 0 1-1V9a5 5 0 1 1 10 0v7.083A2.919 2.919 0 0 1 14.083 19H14a2 2 0 0 0-2-2h-1a2 2 0 0 0-2 2v1a2 2 0 0 0 2 2h1a2 2 0 0 0 1.732-1h.351a4.917 4.917 0 0 0 4.83-4H19a3 3 0 0 0 3-3v-2a3 3 0 0 0-3-3 7 7 0 0 0-7-7Zm1.45 3.275a4 4 0 0 0-4.352.976 1 1 0 0 0 1.452 1.376 2.001 2.001 0 0 1 2.836-.067 1 1 0 1 0 1.386-1.442 4 4 0 0 0-1.321-.843Z" clip-rule="evenodd"/>
           </svg>Contacto</a>
         </div>
       )}
 ...
 {dat.visibilidad_precio === 'NO VISIBLE' && (
   <p className="text-center text-danger font-semibold  bg-[#e3e3e3a9] lg:text-[1rem] text-[0.5rem]">PRECIO NO DISPONIBLE</p>
         ...
         {dat.stock > 0 && (
           <a href={`https://api.whatsapp.com/send?phone=529381958284&text=Hola, estoy interesado en comprar el siguiente equipo: ${dat.nombre}`} target='_blank' className='bg-[#C70000] text-white text-center py-1 rounded-[5px] lg:text-[1rem] text-[0.7rem]'>Rentar equipo</a>
         )}
 ...
 {dat.visibilidad_precio === 'NO VISIBLE' && (
   <p className="text-center text-danger font-semibold lg:text-[1rem] text-[0.5rem] bg-[#e3e3e3a9]">PRECIO NO DISPONIBLE</p>
         ...
         {dat.stock === 0 && (
           <button onClick={()=>{openModal(), setId(dat._id)}} disabled className='bg-[#a5a5a5] text-white py-1 rounded-[5px] lg:text-[1rem] text-danger text-[0.7rem]'>Comprar equipo</button>
         )}
         {dat.stock > 0 && (
           <a href={`https://api.whatsapp.com/send?phone=529381958284&text=Hola, estoy interesado en rentar el siguiente equipo: ${dat.nombre}`} target='_blank' className='bg-[#C70000] text-white text-center py-1 rounded-[5px] lg:text-[1rem] text-[0.7rem]'>Comprar equipo</a>
         )}
         <button onClick={()=>{openModal(), setId(dat._id)}} className='bg-[#323B75] text-white py-1 rounded-[5px] lg:text-[1rem] text-[0.7rem]'>Ficha técnica</button>
       </div>
     ))}
   ...
   <div className="text-center mt-6">
     ...
     <div className='w-full flex flex-col'>
       <div className='bg-[#C70000] py-[2rem] flex items-center px-[0.5rem] lg:gap-0 gap-2 lg:flex-row flex-col lg:px-[4rem] justify-between'>
         <button onClick={()=>{localStorage.setItem('products_venta_current_page',1), window.location.reload()}}><img className='w-[10rem]' src={logo} alt="" /></button>
