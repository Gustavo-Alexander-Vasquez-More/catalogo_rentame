...
export default function index() {
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center w-full h-auto bg-[#e3e2e294]">
        {/* Barra de búsqueda */}
        <div className="flex lg:w-[40%] w-[100%] lg:px-0 px-3 pt-[3rem] justify-center">
  <div className="relative w-full flex items-center">
    <input
      type="text"
      placeholder="Buscar producto ..."
      onKeyDown={handleKeyDown}
      className="w-full lg:text-[1rem] text-[0.9rem] py-3 px-4 border border-gray-300 rounded-l-[7px] focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-md transition duration-300 ease-in-out transform hover:scale-105"
      value={searchTerm}
  ...
  >
    ...
    >
      <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0" />
        ...
        {/* ESTO ES EL BODY */}
        <div className="w-full  px-[0.5rem]  min-h-[80vh]  lg:px-[5rem] py-[2rem] gap-3 flex flex-col">
          {show_paginados === true && (
          ...
          {show_paginados === false && (
            <p className="font-bold text-[1rem] lg:text-[1.7rem] text-secondary bg-gradient-to-r from-[#C70000] to-[#FF5733] text-transparent bg-clip-text drop-shadow-md">
            Resultados de búsqueda ({filteredDatas.length})
          </p>
          )}
  ...
  filteredDatas.map((dat, index) => (
    <div
      key={index}
      className="bg-white w-full px-3 py-4 rounded-lg flex flex-col gap-4 shadow-lg hover:shadow-xl transition duration-300 ease-in-out transform hover:scale-105"
    >
     <a href={`/detalle-producto?id=${dat._id}`} className="group relative">
  <img
    className="w-full h-[12vh] lg:h-[20vh] object-contain rounded-lg transition-all duration-300 group-hover:opacity-60"
    src={dat.foto}
  ...
  <div className="absolute inset-0 bg-black bg-opacity-40 flex justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
    ...
    >
      ...
      <p className="lg:text-[1rem] text-[0.8rem] text-center font-semibold text-gray-800 lg:h-auto h-[40px] line-clamp-2 lg:line-clamp-1">
      ...
      {dat.visibilidad_precio === "NO VISIBLE" && (
        <p className="text-center text-gray-400 invisible font-semibold lg:text-[1rem] text-[0.5rem]">
          MXN
        </p>
      )}
      {dat.stock === 0 && (
        <p className="text-center text-[#D9534F] font-semibold rounded-[5px] py-1 lg:text-[1rem] text-[0.7rem] bg-[#F8D7DA]">
          Rentado
        </p>
      )}
      {dat.stock > 0 && (
        <p className="text-center text-[#28A745] font-semibold rounded-[5px] py-1 lg:text-[1rem] text-[0.7rem] bg-[#D4EDDA]">
          Disponible
        </p>
      )}
      ...
      {dat.stock === 0 && (
        <button
          ...
          disabled
          className="bg-[#B0B0B0] text-white py-2 px-4 rounded-[5px] lg:text-[1rem] text-[0.8rem] cursor-not-allowed"
        >
          Rentar
        </button>
      )}
      {dat.stock > 0 && (
        <a
          ...
          target="_blank"
          className="bg-[#323B75] text-white py-2 px-4 rounded-[5px] lg:text-[1rem] text-[0.8rem] text-center hover:bg-[#5F75B8] transition duration-300 ease-in-out"
        >
          Rentar
        </a>
      )}
      {dat.tipo_uso === "venta" && (
        <a
          ...
          target="_blank"
          className="bg-[#C70000] text-white py-2 px-4 rounded-[5px] lg:text-[1rem] text-[0.8rem] text-center hover:bg-[#D83636] transition duration-300 ease-in-out"
        >
          Comprar equipo
        </a>
      )}
  ...
  <div className="absolute inset-0 bg-black bg-opacity-40 flex justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
    ...
    >
      ...
      />
                  ...
                  <button
        onClick={()=>{setId(dat._id),handleCopy(dat._id)}}
